...
describe('computeDashboardKpis', () => {
  it('counts open, breach, unassigned, category, store, tech', () => {
    ...
    expect(kpis.byCategory[0]).toMatchObject({ key: 'hvac', count: 3 })
    expect(kpis.topStores[0]).toMatchObject({ code: '172', count: 3 })
    expect(kpis.techLoad[0]).toMatchObject({ id: 'tech-a', name: 'Yossi', count: 3 })
    expect(kpis.exceptions.map((t) => t.id)).toEqual(['1', '5'])
    expect(kpis.urgent).toBe(1)
    expect(kpis.urgentTickets.map((t) => t.id)).toEqual(['1'])
    expect(kpis.recentActivity).toHaveLength(5)
  })
})
