...
export function resolveTechId(queryTechId?: string | null): string | null {
  ...
  // Offline demo technician (memory backend)
...
export async function fetchTechTickets(techId: string | null): Promise<{
  ...
  try {
    const { listTechTickets } = await import('@/modules/tech/service')
    const { tickets, backend } = await listTechTickets({ techId })
    return { tickets, fromDb: backend === 'supabase', error: undefined }
  } catch (e) {
    return {
      tickets: [],
      fromDb: false,
...
export async function fetchTechTicket(ticketId: string): Promise<{
  ...
  try {
    const { getTechTicket } = await import('@/modules/tech/service')
    const ticket = await getTechTicket(ticketId)
    return { ticket, fromDb: false }
  } catch (e) {
    return {
      ticket: null,
      fromDb: false,
